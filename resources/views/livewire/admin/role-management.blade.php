<div class="p-6 max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Catálogo de Puestos y Roles</h1>
            <p class="text-sm text-slate-500">Administra los niveles de acceso y cargos para el personal de TerraPago</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-4 border-b border-slate-100 flex items-center justify-between">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Buscar puesto o descripción..." 
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 text-slate-600 text-xs uppercase tracking-wider font-semibold border-b border-slate-200">
                    <th class="py-4 px-6">Puesto / Rol</th>
                    <th class="py-4 px-6 text-center">Usuarios Asignados</th>
            ...
            <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                @forelse ($roles as $rol)
                    <tr class="hover:bg-slate-50/60 transition">
                        <td class="py-4 px-6 font-semibold text-slate-900">
                            {{ $rol->nombre }}
                            @if($rol->descripcion)
                                <p class="text-xs text-slate-400 font-normal mt-0.5">{{ $rol->descripcion }}</p>
                            @endif
                        </td>
                        <td class="py-4 px-6 text-center">
                            <span class="inline-flex items-center justify-center px-3 py-1 text-xs font-bold rounded-full {{ $rol->users_count > 0 ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-500' }}">
                                {{ $rol->users_count }}
                            </span>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="3" class="p-6 text-center text-slate-400">No se encontraron puestos registrados.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
                <h3 class="text-lg font-bold text-slate-800">
                    {{ $modoEdicion ? 'Editar Puesto / Rol' : 'Registrar Nuevo Puesto' }}
                </h3>

                <form wire:submit.prevent="guardar" class="space-y-4">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Nombre del Puesto</label>
                        <input type="text" wire:model="nombre" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="Ej. Cobrador en Campo">
                        @error('nombre') <span class="text-xs text-rose-500">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Descripción</label>
                        <textarea wire:model="descripcion" rows="2" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="Funciones asignadas a este puesto"></textarea>
